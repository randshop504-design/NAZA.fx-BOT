  ...
  try {
    ...
    if (notification.kind === braintree.WebhookNotification.Kind.SubscriptionChargedSuccessfully || kind.includes('subscription_charged_successfully') || kind.includes('subscription_charged')) {
      ...
      if (email && supabase) {
        ...
        console.warn('⚠️ Email no encontrado en webhook');
...
app.get('/health', (_req, res) => res.json({ ok: true, ts: new Date().toISOString(), version: '3.1 - Webhook + Email Mejorado' }));

app.listen(PORT, () => {
  console.log('🟢 NAZA.fx BOT v3.1 - WEBHOOK + EMAIL');
  ...
  console.log('✅ Listo para recibir webhooks');
});
