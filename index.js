...
// Braintree: BraintreeGateway (Node 18+)
...
// Normalización ligera — elimina emojis/pictogramas y limpia espacios
function removeEmojisAndTrim(s){
  if(!s) return '';
  return String(s)
    // heurística amplia para símbolos/emoji (mantener simple y seguro)
    .replace(/([\u231A-\u32FF\uD83C-\uDBFF\uDC00-\uDFFF\u200D\uFE0F])/g, '')
    .replace(/\s+/g,' ')
    .trim();
}
...
// Routes: existing confirm/notify endpoints (frontend -> bot)
...
// NEW: Endpoint to receive tokenized payment_method_nonce and create subscription (bot does the charge)
app.post('/api/payment/process', async (req, res) => {
  try {
    const secret = req.get('X-SHARED-SECRET') || '';
    if (!secret || secret !== SHARED_SECRET) return res.status(401).json({ error: 'unauthorized' });

    const { product_name, plan_id: incoming_plan, payment_method_nonce, email, membership_id, user_name } = req.body || {};
    const plan_id = resolvePlanId({ plan_id: incoming_plan, product_name });

    if (!plan_id || !payment_method_nonce || !email || !membership_id) {
      return res.status(400).json({ error: 'missing_fields_or_plan_not_resolved' });
    }

    const customerId = membership_id;

    // 1) create customer if not exists (ignored if already exists)
    try {
      await gateway.customer.create({ id: customerId, email, firstName: user_name || undefined });
    } catch(e){
      // ignore already exists error
    }

    // 2) create payment method from nonce
    ...
    if (!pmRes.success) {
      return res.status(400).json({ error: 'payment_method_failed', details: pmRes });
    }
    const paymentMethodToken = pmRes.paymentMethod.token;
    ...
    if (!subRes.success) {
      return res.status(400).json({ error: 'subscription_failed', details: subRes });
    }

    const subscription = subRes.subscription;

    // 4) proceed with confirmation actions (email + claim + redirect)
    await handleConfirmedPayment({ plan_id, email, membership_id: subscription.id, user_name });

    return res.json({ ok: true, subscription_id: subscription.id, plan_id });
  } catch (e) { console.error('payment process error', e?.message || e); return res.status(500).json({ error: 'server_error' }); }
});

// Braintree webhook (raw body)
app.post('/api/braintree/webhook', express.raw({ type: '*/*' }), async (req, res) => {
  try {
    ...
    if (notification.kind === braintree.WebhookNotification.Kind.SubscriptionChargedSuccessfully ||
      ...
      if (email && membership_id) { await handleConfirmedPayment({ plan_id: (plan_id || 'plan_unknown'), email, membership_id: subscription.id, user_name }); }
...
// Discord OAuth & role assignment (unchanged)
...
app.listen(PORT, () => {
  ...
  console.log('POST /api/payment/process → expects X-SHARED-SECRET and payment_method_nonce');
