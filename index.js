...
// ============================================
// ENDPOINT: Frontend Confirm (FLUJO CORRECTO)
// ============================================
app.post('/api/frontend/confirm', async (req, res) => {
  try {
    ...
    console.log('🔐 Token esperado:', FRONTEND_TOKEN);
    ...
    // PASO 1: Crear CUSTOMER en Braintree
    console.log('1️⃣ Creando customer en Braintree...');
    ...
    if (!customerResult.success) {
      console.error('❌ Error al crear customer:', customerResult.message);
    ...
    // PASO 2: Crear SUSCRIPCIÓN con el payment method token
    console.log('2️⃣ Creando suscripción...');
    ...
    if (!subscriptionResult.success) {
      console.error('❌ Error al crear suscripción:', subscriptionResult.message);
      return res.status(400).json({ 
...
app.get('/health', (_req, res) => res.json({ ok: true, ts: new Date().toISOString(), version: '5.0 - FIXED: Customer + PaymentMethod + Subscription' }));

app.listen(PORT, () => {
  console.log('🟢 NAZA.fx BOT v5.0 - SUSCRIPCIONES CORREGIDAS');
  console.log('📍 URL:', BASE_URL);
  console.log('🔐 FRONTEND_TOKEN configurado:', FRONTEND_TOKEN ? 'SÍ' : 'NO');
