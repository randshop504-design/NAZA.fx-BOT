// NAZA.fx BOT — FINAL INDEX (Node 18+)
// Whop ↔ Render ↔ Discord + Supabase + Gmail (auto email + role assignment)
require('dotenv').config();

const express = require('express');
const crypto = require('crypto');
const jwt = require('jsonwebtoken');
const nodemailer = require('nodemailer');
const { createClient } = require('@supabase/supabase-js');
const fetch = global.fetch || ((...a) => import('node-fetch').then(({ default: f }) => f(...a)));
const app = express();

/* ========= ENV ========= */
const PORT = process.env.PORT || 3000;
const BASE_URL = process.env.RENDER_EXTERNAL_URL || `http://localhost:${PORT}`;

const APP_NAME = process.env.APP_NAME || 'NAZA Trading Academy';
const DISCORD_BOT_TOKEN = process.env.DISCORD_BOT_TOKEN;
const DISCORD_CLIENT_ID = process.env.DISCORD_CLIENT_ID;
const DISCORD_CLIENT_SECRET = process.env.DISCORD_CLIENT_SECRET;
const DISCORD_REDIRECT_URL = process.env.DISCORD_REDIRECT_URL || `${BASE_URL}/discord/callback`;
const GUILD_ID = process.env.GUILD_ID;
const ROLE_ID = process.env.ROLE_ID;
const WHOP_SIGNING_SECRET = process.env.WHOP_SIGNING_SECRET || process.env.WHOP_WEBHOOK_SECRET;
const JWT_SECRET = process.env.JWT_SECRET || 'change-this-now';
const SUCCESS_URL = process.env.SUCCESS_URL || `${BASE_URL}/redirect`;

const SUPABASE_URL = process.env.SUPABASE_URL;
const SUPABASE_SERVICE_ROLE = process.env.SUPABASE_SERVICE_ROLE;
const supabase = createClient(SUPABASE_URL, SUPABASE_SERVICE_ROLE, { auth: { persistSession: false } });

/* ========= Email ========= */
const {
  GMAIL_USER,
  GMAIL_PASS,
  FROM_EMAIL,
  DISCORD_DOWNLOAD_URL = 'https://discord.com/download',
  DISCORD_TUTORIAL_URL = 'https://youtu.be/_51EAeKtTs0',
  INSTAGRAM_URL = 'https://instagram.com/',
  TIKTOK_URL = 'https://tiktok.com/@',
  WHATSAPP_URL = 'https://wa.me/50400000000',
  TELEGRAM_URL = 'https://t.me/',
  LOGO_URL = '',
  FOOTER_IMAGE_URL = '',
  ADMIN_TEST_TOKEN
} = process.env;

const mailer =
  GMAIL_USER && GMAIL_PASS
    ? nodemailer.createTransport({
        host: 'smtp.gmail.com',
        port: 465,
        secure: true,
        auth: { user: GMAIL_USER, pass: GMAIL_PASS }
      })
    : null;

async function sendEmail(to, { subject, html }) {
  if (!mailer) return console.log('📧 [FAKE EMAIL]', to, subject);
  const info = await mailer.sendMail({
    from: FROM_EMAIL || GMAIL_USER,
    to,
    subject,
    html
  });
  console.log('📧 Enviado:', info.messageId, '→', to);
}

function buildWelcomeEmailHTML({ email, order_id, username = 'Trader' }) {
  const btn = 'display:inline-block;padding:12px 18px;border-radius:10px;text-decoration:none;font-weight:700;';
  const redirectLink = `${BASE_URL}/redirect?email=${encodeURIComponent(email)}&order_id=${encodeURIComponent(order_id)}`;
  return `
  <div style="margin:0;padding:0;background:#0b0d10;">
    <table width="100%" cellpadding="0" cellspacing="0" bgcolor="#0b0d10">
      <tr><td align="center">
        <table width="640" style="max-width:640px;background:#0f1217;color:#e6e9ef;font-family:system-ui,Roboto,Arial;border:1px solid #1b212a;border-radius:12px;overflow:hidden;">
          ${LOGO_URL ? `<tr><td align="center" style="padding:22px"><img src="${LOGO_URL}" alt="logo" style="height:60px;border-radius:12px;"></td></tr>` : ''}
          <tr><td align="center" style="padding:8px 24px 0">
            <h2 style="margin:0;font-size:22px;color:#fff">${APP_NAME}</h2>
            <p style="margin:6px 0 0;font-size:12px;color:#98a1b3">Revisa <b>Spam</b> o <b>Promociones</b> si no ves este correo.</p>
          </td></tr>
          <tr><td style="padding:14px 24px">
            <p>¡Bienvenido, <b>${username}</b>! 🎉</p>
            <p>Para activar tu acceso, acepta los Términos y conecta tu Discord.</p>
            <div style="margin:18px 0">
              <a href="${redirectLink}" style="${btn}background:#2b6cff;color:#fff">Conectar con Discord</a>
            </div>
            <div><a href="${DISCORD_DOWNLOAD_URL}" style="${btn}background:#1f2633;color:#fff">⬇️ Descargar Discord</a></div>
            <div style="margin-top:8px"><a href="${DISCORD_TUTORIAL_URL}" style="${btn}background:#1f2633;color:#fff">▶️ Ver tutorial</a></div>
            <hr style="border:none;border-top:1px solid #2a3240;margin:22px 0">
            <div>
              <a href="${INSTAGRAM_URL}" style="${btn}background:#2a3240;color:#fff;margin-right:8px">📸 Instagram</a>
              <a href="${TIKTOK_URL}" style="${btn}background:#2a3240;color:#fff;margin-right:8px">🎵 TikTok</a>
              <a href="${WHATSAPP_URL}" style="${btn}background:#2a3240;color:#fff;margin-right:8px">💬 WhatsApp</a>
              <a href="${TELEGRAM_URL}" style="${btn}background:#2a3240;color:#fff">📣 Telegram</a>
            </div>
            <p style="color:#9ca3af;font-size:12px">Disclaimer: ${APP_NAME} es educativo; no es asesoría financiera.</p>
          </td></tr>
          ${FOOTER_IMAGE_URL ? `<tr><td><img src="${FOOTER_IMAGE_URL}" alt="banner" style="width:100%;display:block"></td></tr>` : ''}
        </table>
      </td></tr>
    </table>
  </div>`;
}

async function sendAccessEmail({ to, email, order_id, username }) {
  const html = buildWelcomeEmailHTML({ email, order_id, username });
  await sendEmail(to, { subject: `${APP_NAME} — Acceso y pasos (Discord)`, html });
}

/* ========= Supabase helpers ========= */
async function linkGet(id) {
  const { data } = await supabase.from('membership_links').select('membership_id,discord_id').eq('membership_id', id).maybeSingle();
  return data || null;
}
async function linkSet(membership_id, discord_id) {
  await supabase.from('membership_links').upsert({ membership_id, discord_id }, { onConflict: 'membership_id' });
}
async function claimAlreadyUsed(mid, jti) {
  const { error } = await supabase.from('claims_used').insert({ jti, membership_id: mid });
  return error?.code === '23505';
}
async function logWebhook(id, type, data) {
  await supabase.from('webhook_logs').insert({ event_id: id, event_type: type, data });
}

/* ========= Body Parser (raw only for webhook) ========= */
function rawBodySaver(req, _res, buf) { if (buf?.length) req.rawBody = buf; }
app.use((req, res, next) => {
  if (req.path === '/webhook/whop') express.raw({ type: 'application/json', verify: rawBodySaver })(req, res, next);
  else express.json()(req, res, next);
});

/* ========= Página post-pago ========= */
app.get('/redirect', (req, res) => {
  const { email = '', order_id = '', claim = '' } = req.query || {};
  res.type('html').send(`
  <!doctype html><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1">
  <title>${APP_NAME} • Activar acceso</title>
  <style>
    body{margin:0;background:#0b0d10;color:#e6e9ef;font-family:system-ui,Roboto,Arial;text-align:center;padding:40px}
    .box{max-width:500px;margin:auto;background:#11151b;padding:28px;border-radius:14px;border:1px solid #1b212a}
    h2{margin-bottom:8px}button{cursor:pointer}
    .btn{background:#2b6cff;color:#fff;border:0;border-radius:10px;padding:12px 18px;font-weight:700}
    .btn[disabled]{opacity:.5;cursor:not-allowed}
    .warn{margin-top:18px;padding:10px;background:#3a0d0d;border:1px solid #771a1a;border-radius:10px;color:#f3bebe}
  </style>
  <div class="box">
    <h2>Último paso para activar tu acceso</h2>
    <p><b>Compra exitosa.</b> Te enviamos un correo con tus accesos (revisa <b>Spam/Promociones</b>).</p>
    <p><b>${APP_NAME}</b> es educativo; no es asesoría financiera.</p>
    <button id="accept" style="margin-top:10px">✅ Acepto Términos y Condiciones</button><br><br>
    <button id="go" class="btn" disabled>Conectar con Discord</button>
    <div id="warn" class="warn" style="display:none;"></div>
  </div>
  <script>
    let claim = ${JSON.stringify(claim)};
    const order_id=${JSON.stringify(order_id)},email=${JSON.stringify(email)};
    const go=document.getElementById('go'),accept=document.getElementById('accept'),warn=document.getElementById('warn');
    accept.onclick=async()=>{
      const on=accept.dataset.on!=='true';accept.dataset.on=on;go.disabled=!on;
      if(on&&!claim&&order_id&&email){
        const r=await fetch('/api/claim/issue',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({order_id,email})});
        const j=await r.json();if(j.claim)claim=j.claim;
      }
    };
    go.onclick=()=>{
      if(go.disabled)return;
      if(!claim){warn.style.display='block';warn.textContent='Aún no tenemos tu enlace seguro. Usa tu correo para generarlo o revisa tu email.';return;}
      window.location.href='/discord/login?claim='+encodeURIComponent(claim);
    };
  </script>`);
});

/* ========= Seguridad ========= */
function requireClaim(req, res, next) {
  try { req.claim = jwt.verify(req.query.claim, JWT_SECRET); next(); }
  catch { res.status(401).send('⛔ Enlace inválido o usado.'); }
}

/* ========= Discord OAuth ========= */
app.get('/discord/login', requireClaim, (req, res) => {
  const state = jwt.sign(req.claim, JWT_SECRET, { expiresIn: '10m' });
  const params = new URLSearchParams({
    client_id: DISCORD_CLIENT_ID,
    redirect_uri: DISCORD_REDIRECT_URL,
    response_type: 'code',
    scope: 'identify guilds.join',
    prompt: 'consent',
    state
  });
  res.redirect('https://discord.com/api/oauth2/authorize?' + params);
});

app.get('/discord/callback', async (req, res) => {
  try {
    const { code, state } = req.query;
    const st = jwt.verify(state, JWT_SECRET);
    const t = await fetch('https://discord.com/api/oauth2/token', {
      method: 'POST',
      headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
      body: new URLSearchParams({
        client_id: DISCORD_CLIENT_ID,
        client_secret: DISCORD_CLIENT_SECRET,
        grant_type: 'authorization_code',
        code,
        redirect_uri: DISCORD_REDIRECT_URL
      })
    });
    const { access_token } = await t.json();
    const me = await (await fetch('https://discord.com/api/v10/users/@me', { headers: { Authorization: `Bearer ${access_token}` } })).json();
    if (await claimAlreadyUsed(st.membership_id, st.jti)) return res.status(409).send('⛔ Enlace ya usado.');
    const existing = await linkGet(st.membership_id);
    if (existing?.discord_id && existing.discord_id !== me.id) return res.status(403).send('⛔ Ya vinculado.');
    await fetch(`https://discord.com/api/v10/guilds/${GUILD_ID}/members/${me.id}`, {
      method: 'PUT', headers: { Authorization: 'Bot '+DISCORD_BOT_TOKEN, 'Content-Type':'application/json' },
      body: JSON.stringify({ access_token })
    });
    await fetch(`https://discord.com/api/v10/guilds/${GUILD_ID}/members/${me.id}/roles/${ROLE_ID}`, {
      method:'PUT',headers:{Authorization:'Bot '+DISCORD_BOT_TOKEN}
    });
    await linkSet(st.membership_id, me.id);
    res.redirect(SUCCESS_URL);
  } catch (e) {
    console.error('DISCORD_CALLBACK_ERROR', e.message); res.status(500).send('OAuth error');
  }
});

/* ========= Claim API ========= */
app.post('/api/claim/issue', async (req, res) => {
  try {
    const { order_id, email } = req.body;
    const since = new Date(Date.now() - 20*60*1000).toISOString();
    const { data } = await supabase.from('webhook_logs').select('event_type,data,received_at').gte('received_at', since);
    const okEvents = ['payment_succeeded','membership_activated','membership_went_valid'];
    const found = (data||[]).find(r=>{
      const d=r.data?.data||r.data;
      const em=(d?.user?.email||d?.email||'').toLowerCase();
      const id=d?.id||d?.membership_id||d?.order_id||'';
      return okEvents.includes(r.event_type)&&em===email.toLowerCase()&&id===order_id;
    });
    if(!found)return res.status(404).json({error:'no_payment'});
    const d=found.data?.data||found.data;
    const claim=jwt.sign({membership_id:d.id||order_id,whop_user_id:d.user?.id||'U',jti:crypto.randomUUID()},JWT_SECRET,{expiresIn:'24h'});
    res.json({claim});
  }catch(e){res.status(500).json({error:e.message});}
});

/* ========= Webhook verify (HMAC patch) ========= */
function verifyWhopV1(req){
  const sigHeader=req.get('Whop-Signature')||req.get('X-Whop-Signature')||req.get('Whop-Webhook-Signature');
  if(!WHOP_SIGNING_SECRET)return true;
  if(!sigHeader)return false;
  let provided='';
  if(sigHeader.includes('=')){
    const parts=Object.fromEntries(sigHeader.split(',').map(s=>{
      const[k,...r]=s.trim().split('=');
      return[k,r.join('=')];
    }));
    provided=(parts.v1||parts.V1||'').trim();
  }else provided=sigHeader.trim();
  const expected=crypto.createHmac('sha256',WHOP_SIGNING_SECRET).update(req.rawBody).digest('hex');
  try{
    const A=Buffer.from(expected,'utf8'),B=Buffer.from(provided,'utf8');
    if(A.length!==B.length)return false;
    return crypto.timingSafeEqual(A,B);
  }catch{return false;}
}

/* ========= Webhook Handler ========= */
const okEvents=new Set(['payment_succeeded','membership_activated','membership_went_valid']);
const cancelEvents=new Set(['membership_cancelled','membership_cancelled_by_user','membership_expired','membership_deactivated']);

app.post('/webhook/whop',async(req,res)=>{
  try{
    if(!verifyWhopV1(req)){console.log('⛔ invalid_signature');return res.status(401).json({error:'invalid_signature'});}
    const body=JSON.parse(req.rawBody.toString('utf8'));
    const action=body?.action||body?.event||'unknown';
    const id=body?.id||crypto.randomUUID();
    await logWebhook(id,action,body);
    const email=body?.data?.user?.email||body?.data?.email;
    const memberId=body?.data?.id||body?.data?.membership_id;
    if(okEvents.has(action)&&email){
      await sendAccessEmail({to:email,email,order_id:memberId,username:body?.data?.user?.username||'Trader'});
      return res.json({status:'claim_email_sent'});
    }
    if(cancelEvents.has(action)&&memberId){
      const linked=await linkGet(memberId);
      if(linked?.discord_id){
        await fetch(`https://discord.com/api/v10/guilds/${GUILD_ID}/members/${linked.discord_id}/roles/${ROLE_ID}`,{
          method:'DELETE',headers:{Authorization:'Bot '+DISCORD_BOT_TOKEN}
        });
      }
      return res.json({status:'role_revoked'});
    }
    res.json({status:'ignored'});
  }catch(e){console.error('WHOP_WEBHOOK_ERROR',e.message);res.status(500).json({error:'server_error'});}
});

/* ========= Debug Routes ========= */
app.get('/smtp-verify',async(_req,res)=>{
  try{
    if(!GMAIL_USER||!GMAIL_PASS)return res.send('Mailer inactivo');
    const t=nodemailer.createTransport({host:'smtp.gmail.com',port:465,secure:true,auth:{user:GMAIL_USER,pass:GMAIL_PASS}});
    await t.verify();res.send('SMTP OK ✅');
  }catch(e){res.status(500).send('SMTP ERROR: '+e.message);}
});

app.get('/debug/webhook-logs',async(_req,res)=>{
