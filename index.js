...
// NOTA: NO se modificaron plantillas/funciones de correo de bienvenida (sendWelcomeEmail) — se usan igual.
...
/* ===========================
   REEMPLAZO ROBUSTO removeDiscordRole
   - Primero intenta la API REST DELETE roles (como antes) con logging extenso.
   - Si falla, hace fallback con discord.js: fetch guild, fetch member, imprime roles del miembro y del bot,
     verifica jerarquía (posición) y luego intenta member.roles.remove(roleId).
   - Devuelve true si remueve, false si no.
   =========================== */
async function removeDiscordRole(discordId, roleId) {
  if (!discordId || !roleId) {
    console.warn('removeDiscordRole: falta discordId o roleId', { discordId, roleId });
    return false;
  }
  if (!GUILD_ID) {
    console.warn('removeDiscordRole: GUILD_ID no configurado; no se puede remover rol');
    return false;
  }

  // 1) Intentar via API REST (DELETE)
  try {
    const apiUrl = `https://discord.com/api/v10/guilds/${GUILD_ID}/members/${discordId}/roles/${roleId}`;
    ...
    let body = '<no-body>';
    try { body = await apiResp.text(); } catch(e){ /* ignore */ }
    console.log(`DEBUG removeRoleViaApi -> status: ${status}, body: ${String(body).substring(0,400)}`);

    if (status === 204) {
      console.log(`✅ removeRoleViaApi: rol ${roleId} removido de ${discordId}`);
      return true;
    } else {
      console.warn(`⚠️ removeRoleViaApi responded ${status}: ${body}. Intentando fallback con discord.js`);
    }
  } catch (err) {
    console.error('❌ removeRoleViaApi error:', err);
  }

  // 2) Fallback con discord.js: fetch guild, fetch member, mostrar roles y remover.
  try {
    const guild = await discordClient.guilds.fetch(GUILD_ID);
    if (!guild) {
      console.warn('removeDiscordRole: guild no encontrada con id', GUILD_ID);
      return false;
    }
    ...
    try {
      ...
      console.warn('removeDiscordRole: no se pudo obtener member fetch(discordId):', fetchErr?.message || fetchErr);
      // si member no está en el guild -> nada que hacer (devuelve false)
      return false;
    }

    // log: roles del miembro
    const memberRoles = member.roles.cache.map(r => ({ id: r.id, name: r.name, position: r.position }));
    console.log('DEBUG member.roles:', memberRoles);

    // log: rol del bot (para jerarquía)
    ...
    console.log('DEBUG bot.roles:', botRoles);

    // comprobar si el bot puede administrar roles (jerarquía)
    const botMaxPos = Math.max(...botRoles.map(r => r.position), 0);
    const targetRole = guild.roles.cache.get(roleId);
    const targetPos = targetRole ? targetRole.position : null;
    console.log('DEBUG roleId position:', { roleId, targetPos, botMaxPos });

    if (targetPos !== null && botMaxPos <= targetPos) {
      console.error('❌ removeDiscordRole: la jerarquía del bot es insuficiente. El rol objetivo está por encima o igual al maxRole del bot.');
      return false;
    }

    // intentar remover el rol
    try {
      await member.roles.remove(roleId);
      console.log(`✅ removeDiscordRole: rol ${roleId} removido de ${discordId} via discord.js`);
      return true;
    } catch (errRemove) {
      console.error('❌ Error removiendo rol con discord.js:', errRemove);
      return false;
    }
  } catch (err) {
    console.error('❌ removeDiscordRole fallback error:', err);
    return false;
  }
}
...
// ============================================
// EMAIL DE EXPIRACIÓN / REACTIVACIÓN (HTML + envío)
...
// ============================================
// CALLBACK DE DISCORD OAUTH2 -> /discord/callback (robusto - mantengo tu lógica)
app.get('/discord/callback', async (req, res) => {
  try {
    ...
    // --- 1) Intercambio code -> token (simple, como tenías)
    let tokenData = null;
    try {
      ...
      try {
        ...
        console.error('Token exchange devolvió body no-JSON. status:', tokenResp.status, 'body starts with:', (tokenText||'').substring(0,120));
        return res.status(400).send('Error de autorización (token exchange).');
      }
    ...
    try {
      ...
      try {
        ...
        console.error('Fetch user devolvió body no-JSON. status:', userResp.status, 'body starts with:', (userText||'').substring(0,120));
    ...
    // --- 5) UPDATE condicional (guardar discord_id y marcar used) ---
    try {
      const updates = {
      ...
      if (membership) {
        ...
        // crear fila mínima si no existía (no ideal pero cubre casos edge)
        const row = {
          claim: state,
...
// ============================================
// EXPIRACIONES AUTOMÁTICAS (QUITAR ROL y ENVIAR EMAIL de reactivación)
  ...
  try {
    ...
    for (const m of rows) {
      try {
        ...
        // 1) Intentar quitar rol (sin expulsar) y log detallado del resultado
        if (m.discord_id) {
          try {
            console.log(`Intentando remover rol para membership ${m.id} -> discord_id=${m.discord_id}, roleId=${roleId}`);
            const removed = await removeDiscordRole(m.discord_id, roleId);
            if (removed) {
              console.log(`✅ Membership ${m.id}: rol ${roleId} removido correctamente de ${m.discord_id}`);
            } else {
              console.warn(`⚠️ Membership ${m.id}: no se pudo remover rol ${roleId} de ${m.discord_id}. Revisar logs previos para causa.`);
            }
          } catch (err) {
            console.error('removeDiscordRole error (loop):', err);
          }
        } else {
          console.log('Membership no tiene discord_id; no se puede remover rol.');
        }
        ...
        // 3) Enviar email de expiración / reactivación (si el email existe)
        try {
...
setTimeout(() => {
  ...
  // ejecutar expireMemberships cada 60 segundos (1 minuto)
